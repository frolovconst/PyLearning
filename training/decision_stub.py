...
def read_input(sort):
    on_server = exists('stump.in')
    base_path = './' if on_server else 'data/stump/'
    with open(base_path + 'stump.in', 'r') as f:
        n = int(f.readline())
        if n == 0:
            result = None
        else:
...
# def test_dyns():
#     dyn = DynamicSSE()
#     # print(np.arange(1).mean())
#     for i in range(10):
#         example = np.arange(i + 1)
#         true = np.sum(np.square(example.mean() - example))
#         dyn.add(i)
#         pred = dyn.sse
#         print(f'{true == pred} {pred=}', example)
#
